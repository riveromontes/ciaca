<div class="form-group">
  ...
  <select id="id_estudiante" name="id_estudiante" class="form-control">
    ...
    @foreach($estudiantes as $estudiante)
       <option value="{{$estudiante->id}}">{{$estudiante->nombre.' '.$estudiante->apellido.'-'.$estudiante->cedula}}</option>
    @endforeach
  </select>
</div>
...
<div class="form-group">
  ...
  <select id="id_instructor" name="id_instructor" class="form-control">
    ...
    @foreach($instructors as $instructor)
       <option value="{{$instructor->id}}">{{$instructor->nombre.' '.$instructor->apellido.'-'.$instructor->cedula}}</option>
    @endforeach
  </select>
</div>
...
<div class="form-group d-none">
  ...
  {{ Form::text('horas_helice', 0, ['class' => 'form-control']) }}
</div>
<div class="form-group d-none">
  {{ Form::label('horas_aceite', 'Horas de aceite') }}
  {{ Form::text('horas_aceite', 0, ['class' => 'form-control']) }}
</div>
<div class="form-group d-none">
  {{ Form::label('horas_fuselaje', 'Horas de fuselaje') }}
  {{ Form::text('horas_fuselaje', 0, ['class' => 'form-control']) }}
</div>
<div class="form-group d-none">
  {{ Form::label('horas_bujias', 'Horas de bujía') }}
  {{ Form::text('horas_bujias', 0, ['class' => 'form-control']) }}
</div>
<div class="form-group">
